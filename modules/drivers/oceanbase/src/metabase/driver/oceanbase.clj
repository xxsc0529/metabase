...
(defmethod driver-api/spec :oceanbase
  [_ {:keys [host port db]
      :as   opts}]
  ;; Only create spec if we have valid connection details
  (when (and host port db (not (str/blank? host)) (not (str/blank? db)))
    (merge
     {:classname   "com.oceanbase.jdbc.Driver"
      :subprotocol "oceanbase"
      :subname     (metabase.app-db.spec/make-subname host port db)}
...
(defmethod sql.qp/quote-style :oceanbase
  [driver]
  ;; For OceanBase, we need to determine the quote style based on the actual connection mode
  ;; Since this method is called during query processing, we'll use a smart approach
  ;; that tries to detect the mode from the current connection context
  (try
    ;; Try to get the current database context to determine the mode
    (if-let [metadata-provider (driver-api/metadata-provider)]
      (let [database (driver-api/database metadata-provider)
        ...
        (case (clojure.string/lower-case mode)
          "mysql" :mysql
          :oracle))
      ;; If no database context available, default to oracle mode for safety
      :oracle)
    (catch Exception e
      (log/debug "Error determining OceanBase quote-style, defaulting to oracle. Error:" (.getMessage e))
      :oracle)))
...
(defmethod sql.qp/apply-top-level-clause [:oceanbase :limit]
  ...
  (letfn [(oracle-limit [query val]
    ...
    ;; Try to determine the mode from the current database context
    (try
      (if-let [metadata-provider (driver-api/metadata-provider)]
        (let [database (driver-api/database metadata-provider)
              details (:details database)
        ...
        (oracle-limit honeysql-query value))
      (catch Exception e
        (log/debug "Error determining OceanBase mode for limit, defaulting to Oracle mode. Error:" (.getMessage e))
        (oracle-limit honeysql-query value)))))
